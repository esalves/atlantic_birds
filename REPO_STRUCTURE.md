...
| `passer90.rda`, `passer90_export.csv`, `passer90_climate.rds`, `dat_no_grassland.rds`, `dat_no_ants.rds`, `dat_test.rds` | `Analysis/data/derived/` |
| `data_summary.rds`, `descriptive_summary.rds`, `diet_interaction_results.rds`, `arthropod_estimates.rds`, `sensitivity_thresholds_tier1.rds`, `drmsem_results_*.{rds,md}`, drmSEM caches | `Analysis/output/` |
| `brm0_multiphylo.rda`, `brm_bill_multiphylo.rda`, `brm_no_grass.rda`, `brm_no_ants_no_grass.rda`, `brm_arthro_abund*.rda`, `brm_climate.rda` | `Analysis/output/models/` |
| `sensitivity_thresholds_tier1.png`, `drmsem_*_{arthro,noarthro}.png` | `Analysis/figures/` |

## Archived (moved out of the active pipeline)

- `explore_biotime.R`, `biotime_brazil_arthropods.rds`, `biotime_plots/`, `biotime_cache/` → `archive/biotime/` (BioTIME exploration; the manuscript uses PREDICTS, not BioTIME)
