...
Decisions already applied (see CODE_REVIEW.md):
  - Body-size (wing-length) result is now the coalesced-variable model fitted
    ...
    (excludes zero); N and species count auto-populated from data_summary.rds.
    STALE — re-run atlantic_parallel.R after threshold change (n≥30, range≥5) to
    update β/CI/phylogenetic signal. Supersedes the earlier BirdTree estimate.
...
![Wing length of `r dat_sum$n_spp` Atlantic Forest passerines against year of sampling, 1990–2018. Shading shows the density of individual records (log scale); the line is the model-estimated population trend with its 95% interval [UPDATE: rerun atlantic_parallel.R].](images/fig-wingtrend.png){#fig-wingtrend}
