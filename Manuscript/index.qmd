...
Decisions already applied (see CODE_REVIEW.md and LIVE_ONLY_MIGRATION.md):
  - LIVE-ONLY SAMPLE (2026-06): all main analyses now use only live-captured
    birds (Status == "live"); museum specimens are dropped because they shrink/
    shift on preservation and cluster in the early years (confound). The live
    filter is applied before the species thresholds. Sample is now 73 species /
    12,571 records (1995-2018), down from 89 / 15,332.
  - Body-size (wing-length) result is the coalesced-variable model fitted
    ...
    (atlantic_parallel.R), re-run on the live-only sample (thresholds n≥30,
    range≥5: 73 species / 12,571 records): year effect beta = -0.92, 95% CI
    [-1.40, -0.44] (excludes zero). All quoted descriptive, quartile, lnCVR and
    pooled-model numbers are auto-populated from descriptive_summary.rds
    (built by update_descriptive_stats.R), which now also supplies n_spp/n_rec
    (data_summary.rds is deprecated and no longer read here). Supersedes the
    earlier BirdTree estimate.
  - NOTE: on the live-only sample bill width still shows a slight INCREASE over
    time (beta = +0.24 [0.06, 0.43], CI excludes zero), not a null — text/
    abstract updated accordingly.
  - drmSEM: the FULL-RECORD model (no arthropod node, 1995-2018) is now the
    interpreted causal model (Fisher's C clean, P ~ 0.78); the arthropod-
    coverage variant (1998-2008) is REJECTED (Fisher's C P = 0.009) and is
    reported only as a qualitative check. Results/Methods rewritten accordingly.
...
Still needs you:
  - REGENERATE THE FIGURES on the live-only sample. fig-map, fig-wingtrend,
    fig-trends, fig-variability and diet_interaction_plot are built by
    make_figures.py and still reflect the OLD 89-species sample; rerun it so
    the panels match the 73-species numbers. fig-arthropods is a placeholder
    pending regeneration (PREDICTS auto-downloads via predictsr; arthropods are
    unaffected by the live-only filter). fig-drmsem has already been swapped to
    the full-record DAG (Analysis/figures/drmsem_dag_noarthro.png).
...
We fitted the graph primarily on the full record (`r drm_f$year_range[1]`–`r drm_f$year_range[2]`; N = `r .n(drm_f$n_records)` records, `r drm_f$n_species` species), which preserves the complete time series. At this scope the hypothesised graph was consistent with the data (Fisher's C = `r round(drm_f$fisher_c$fisher_c, 1)`, df = `r drm_f$fisher_c$df`, P = `r round(drm_f$fisher_c$p.value, 2)`). Within it, temperature's direct association with the wing-length *mean* could not be distinguished from zero (path = `r .dpath(drm_f, "scaled_tmean", "wing_length")`, P `r .dp(drm_f, "scaled_tmean", "wing_length")`; total temperature effect `r .deff(drm_f, "tmean", "total_path")` mm, 95% CI `r .deff(drm_f, "tmean", "total_path", "conf.low")` to `r .deff(drm_f, "tmean", "total_path", "conf.high")`), and the year → wing route through temperature was negligible (`r .deff(drm_f, "yr_via_tmean", "total_path")` mm, 95% CI `r .deff(drm_f, "yr_via_tmean", "total_path", "conf.low")` to `r .deff(drm_f, "yr_via_tmean", "total_path", "conf.high")`). The model therefore does not localise the modest *mean* decline to the measured climate mediator.
...
![Distributional piecewise structural equation model (drmSEM) of the hypothesised causal graph, fitted on the full record. Arrows are mean-model paths coloured by standardised coefficient (green positive, red negative; solid = P < 0.05, dashed = not significant); dotted purple arcs are the distributional (σ) paths into wing length. Neither year nor temperature has a clear association with the wing-length mean, but the residual variability of wing length rises with year and falls in warmer years. Node roles: grey = exogenous, green = mediator, orange = response.](images/fig-drmsem.png){#fig-drmsem}
